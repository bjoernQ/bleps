...
pub mod asynch {
    ...
    pub struct AsyncHciConnector<T>
    where
        T: embedded_io_async::Read + embedded_io_async::Write,
    {
        hci: RefCell<T>,
    ...
    impl<T> AsyncHciConnector<T>
    where
        T: embedded_io_async::Read + embedded_io_async::Write,
    {
        pub fn new(hci: T, get_millis: fn() -> u64) -> AsyncHciConnector<T> {
            AsyncHciConnector {
                hci: RefCell::new(hci),
                get_millis,
            }
    ...
    impl<T> AsyncHciConnection for AsyncHciConnector<T>
    where
        T: embedded_io_async::Read + embedded_io_async::Write,
    {
        async fn read(&self) -> Option<u8> {
            let mut buf = [0u8];
            let res = self.hci.borrow_mut().read(&mut buf).await;
            match res {
                Ok(len) if len == 1 => Some(buf[0]),
                _ => None,
            }
        ...
        async fn write(&self, data: u8) {
            self.hci.borrow_mut().write(&[data]).await.ok();
        }
