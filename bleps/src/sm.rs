...
impl SecurityManager {
    pub(crate) fn handle(&mut self, ble: &mut Ble, src_handle: u16, payload: crate::Data) {
        ...
        match command {
            SM_PAIRING_REQUEST => {
                self.handle_pairing_request(ble, src_handle, data);
            }
            SM_PAIRING_PUBLIC_KEY => {
                self.handle_pairing_public_key(ble, src_handle, data);
            }
            SM_PAIRING_RANDOM => {
                self.handle_pairing_random(ble, src_handle, data);
            }
            SM_PAIRING_DHKEY_CHECK => {
                self.handle_pairing_dhkey_check(ble, src_handle, data);
            }
            // handle FAILURE
            _ => {
    ...
    fn handle_pairing_request(&mut self, ble: &mut Ble, src_handle: u16, _data: &[u8]) {
        ...
        write_sm(ble, src_handle, data);
    }

    fn handle_pairing_public_key(&mut self, ble: &mut Ble, src_handle: u16, pka: &[u8]) {
    ...
    fn handle_pairing_random(&mut self, ble: &mut Ble, src_handle: u16, random: &[u8]) {
    ...
    fn handle_pairing_dhkey_check(&mut self, ble: &mut Ble, src_handle: u16, ea: &[u8]) {
...
fn write_sm(ble: &mut Ble, handle: u16, data: Data) {
